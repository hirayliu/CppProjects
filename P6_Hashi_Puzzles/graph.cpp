...
void Edge::assert_edge(const Point &a_, const Point &b_) {
...
Node::Node(const Point &p, int n) : pt(p),num_paths(n) {
    ...
    // assert (n >= 1 && n <= 8);
    assert (n >= 0 && n <= 8);      // 0 is for debug printing
}

// ====================================================================================

// helper function for printing Edge
std::ostream& operator<<(std::ostream &ostr, const Edge &e) {
    ostr << "    Edge: " << e.get_a() << "-" << e.get_b();
    return ostr;
}
...
std::ostream& operator<<(std::ostream &ostr, const std::vector<Node> &nodes) {
    for( unsigned int i=0; i<nodes.size(); i++ )
        ostr << nodes[i].get_pt() << ", " << nodes[i].get_num_paths() << "; ";
    return ostr;
}
